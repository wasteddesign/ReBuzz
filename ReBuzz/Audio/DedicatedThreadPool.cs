...
namespace Helios.Concurrency
{
    ...
    internal sealed class DedicatedThreadPool : IDisposable
    {
        ...
        public DedicatedThreadPool(DedicatedThreadPoolSettings settings)
        {
            ...
            Settings = settings;
            _workers = Enumerable.Range(0, settings.NumThreads).Select(workerId => new PoolWorker(this, workerId)).ToArray();
        ...
        private class PoolWorker
        {
            ...
            public PoolWorker(DedicatedThreadPool pool, int workerId)
            {
                ...
                thread.Priority = ProcessAndThreadProfile.DedicatedThreadPoolThread;
                thread.Start();
            }

            private void RunThread()
            {
                //CpuAffinity.SetCurrentThreadToHaveCpuAffinityFor(_workerId);
        ...
        private sealed class UnfairSemaphore
        {
            ...
            private static readonly int ProcessorCount = 8;// Environment.ProcessorCount;
...
#pragma warning restore 169
            ...
            public bool Wait(TimeSpan timeout)
            {
                ...
                //
                int numSpins = 0;
                const int spinLimitPerProcessor = 0;// 50;
                while (true)
                {
                    SemaphoreState currentCounts = GetCurrentState();
                    SemaphoreState newCounts = currentCounts;
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            // Thread.Sleep(0) causes UI thread freeze?
                            //Thread.Yield();
