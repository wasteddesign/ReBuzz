...
{
    internal class ChangeRateAction : WaveAction
    {

        public ChangeRateAction(WaveformVM waveformVm, object x) : base(waveformVm)
        {
            Ready = false;
            SaveState(x, true);
            InputDialogNumber inputDialogNumber = new InputDialogNumber(Effects.GetBuzzThemeResources(), "Change rate (percents)", 0, -50, 100, 1, (decimal)0.1);
        ...
        protected override void DoAction()
        {
            if (UpdateFromState(false))
            {
                if (WaveformVm.Waveform != null)
                {
                    ...
                    Effects.ChangeRate(Wavetable, selectedSlotIndex, semiTones, sequenceMs, seekWindowMs, overlapMs);
