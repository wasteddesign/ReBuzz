...
namespace WDE.ModernPatternEditor
{
    public static class PatternEditorUtils
    {
        ...
        public static List<MPEPattern> ProcessEditorData(Editor editor, byte[] data)
        {
        ...
        private static List<MPEPattern> ParsePatternXPData(Editor editor, byte[] machinePatternsData, int index, byte version)
        {
            List<MPEPattern> patterns = new List<MPEPattern>();
        ...
        private static List<MPEPattern> ParseModernPatternEditorData(Editor editor, byte[] machinePatternsData, int index, byte version)
        {
            List<MPEPattern> patterns = new List<MPEPattern>();
            int numpat = BCReadInt(machinePatternsData, ref index);
        ...
        {
            // MPE Beats
            data.AddRange(BitConverter.GetBytes((int)(pat.Pattern.Length + BUZZ_TICKS_PER_BEAT - 1) / BUZZ_TICKS_PER_BEAT));
            data.AddRange(BitConverter.GetBytes(pat.RowsPerBeat));
        ...
        private static readonly int BUZZ_TICKS_PER_BEAT = 4;
