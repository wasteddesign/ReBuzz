...
namespace WDE.ModernPatternEditor
{
    internal class PlayRecordManager
    {
        ...
        public PlayRecordManager(Editor editor)
        {
        ...
        {
            lock (PianorollEditor.syncLock)
            {
                if (Global.Buzz.Song != null)
                {
        ...
        {
            ReadOnlyCollection<ISequence> seqs = GetPlayingSequences(PianorollEditor.PianorollMachine.Machine);
            ...
            {
                int playPosition = GetPositionInPattern(out pat, seq, PianorollEditor.PianorollMachine.Machine);
                ...
                if (playPosition != -1 && pat != null)
                {
                    ...
                    {
                        prevPlayPos = (pat.Length * PatternEvent.TimeBase + playPosition) % (pat.Length * PatternEvent.TimeBase);
                    }
                ...
                {
                    var mpePattern = PianorollEditor.MPEPatternsDB.GetMPEPattern(pat);
                }
            }
        ...
        private void FireCollectedEvents(List<CollectedEventState> collectEvents)
        {
            ...
            foreach (var e in collectEvents)
            {
                ...
                PianorollEditor.PianorollMachine.ControlChange(machine, group, track, param, value);
            }
        ...
        {
            var mpep = PianorollEditor.MPEPatternsDB.GetMPEPattern(pat);

            foreach (var mpeColumn in mpep.MPEPatternColumns)
            {
                if (PianorollEditor.MPEPatternsDB.IsColumnMuted(mpeColumn.Parameter, mpeColumn.ParamTrack))
                    continue;
        ...
        internal void Init()
        {
            if (PianorollEditor.TargetMachine == null)
                return;

            lock (PianorollEditor.syncLock)
            {
                PlayingNotesList = new List<PlayingTrackInfo>(PianorollEditor.TargetMachine.TrackCount);
                ...
                if (song != null)
                {
                    for (int i = 0; i < playInfoDictionary.Keys.Count; i++)
                    {
                        ...
                        info.PreviousPosition = song.PlayPosition;
                    }
                }
            }
        }
        ...
        {
            var vmPattern = PianorollEditor.SelectedMachine.Patterns.FirstOrDefault(x => x.Pattern == pat);
                    ...
                    if (parameterColumn.PatternColumn.Parameter == parameter && parameterColumn.PatternColumn.Track == track)
                    {
                        ...
                        Application.Current.Dispatcher.BeginInvoke((Action)(() =>
                        {
                            ...
                            foreach (var a in parameterColumn.EditValue(digit, value))
                                PianorollEditor.EditContext.ActionStack.Do(a);
                        }));
                    }
                }
            }
        ...
        {
            foreach (var pattern in PianorollEditor.SelectedMachine.Machine.Patterns)
            {
        ...
        private void RecordPlayingMidiNotePattern(IPattern pattern, int playPosition, int value, int velocity, int channel)
        {
            ...
            if (machinePatternColumnData.midiNote.column != null)
            {
                ...
                {
                    using (new ActionGroup(PianorollEditor.EditContext.ActionStack))
                    {
                        // Note
                        if (machinePatternColumnData.midiNote.column != null)
                        {
                            ...
                            PianorollEditor.DoAction(new MPESetOrClearEventsAction(pattern, mpeColumn, new PatternEvent[] { new PatternEvent() { Time = timeq, Value = buzzNote } }, true));
                        }
                        // Volume
                        if (machinePatternColumnData.midiVelocity.column != null)
                        {
                            ...
                            PianorollEditor.DoAction(new MPESetOrClearEventsAction(pattern, mpeColumn, new PatternEvent[] { new PatternEvent() { Time = timeq, Value = vol } }, true));
                        }
                        // Delay
                        if (machinePatternColumnData.midiDelay.column != null)
                        {
                            ...
                            PianorollEditor.DoAction(new MPESetOrClearEventsAction(pattern, mpeColumn, new PatternEvent[] { new PatternEvent() { Time = timeq, Value = delay } }, true));
                        }
                        ...
                        if (machinePatternColumnData.midiChannel.column != null)
                        {
                            ...
                            int timeq = mpeColumn.GetTimeQuantized(playPosition);
                            PianorollEditor.DoAction(new MPESetOrClearEventsAction(pattern, mpeColumn, new PatternEvent[] { new PatternEvent() { Time = timeq, Value = channel + 1 } }, true));
                        }
                    }
                });
            ...
            if (machinePatternColumnData.midiNote.column != null)
            {
                    ...
                    if (machinePatternColumnData.midiCut.column != null && playPosition > noteTimeQ + delay)
                    {
                        ...
                        if (columnCut != null)
                        {
                            ...
                            if (cutEvents.Count() == 0)
                            {
                                ...
                                Application.Current.Dispatcher.Invoke(() =>
                                {
                                    PianorollEditor.DoAction(new MPESetOrClearEventsAction(pattern, columnCut, new PatternEvent[] { new PatternEvent() { Time = cutTimeQ, Value = cutTime } }, true));
                                });
                            }
                        }
                    }
                ...
                else
                {
                    Application.Current.Dispatcher.Invoke(() =>
                    {
                        using (new ActionGroup(PianorollEditor.EditContext.ActionStack))
                        {
                            PianorollEditor.DoAction(new MPESetOrClearEventsAction(pattern, columnNote, new PatternEvent[] { new PatternEvent() { Time = noteTimeQ, Value = BuzzNote.Off } }, true));


                            if (columnDelay != null)
                            {
                                int delayTime = columnDelay.GetRelativeTimeInRowToParamValue(playPosition);
                                PianorollEditor.DoAction(new MPESetOrClearEventsAction(pattern, columnDelay, new PatternEvent[] { new PatternEvent() { Time = delayTimeQ, Value = delayTime } }, true));
                            }
                            ...
                            if (columnChannel != null)
                            {
                                ...
                                PianorollEditor.DoAction(new MPESetOrClearEventsAction(pattern, columnChannel, new PatternEvent[] { new PatternEvent() { Time = channelTimeQ, Value = channel + 1 } }, true));
                            }
                        }
                    });
        ...
        */
        ...
        private MidiEventData GetMidiDataParameterIndexes(IPattern pattern, IMachine machine, int track)
        {
            ...
            var mpePattern = PianorollEditor.MPEPatternsDB.GetMPEPattern(pattern);
        ...
        internal void UpdatePlayingMidiNote(int channel, int value, int velocity)
        {
            ...
            else if (PianorollEditor.SelectedMachine != null)
            {
                PlayingTrackInfo pti = new PlayingTrackInfo();
                pti.note = buzzNote;
                pti.track = -1;
                pti.channel = channel;
                pti.parameter = PianorollEditor.SelectedMachine.Machine.ParameterGroups[0].Parameters[0]; // Just get a parameter
